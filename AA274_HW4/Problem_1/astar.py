...
class AStar(object):
    ...
    def get_neighbors(self, x):
        ...
        Neighbour_List = []
        Resolution_Degree = np.array((-1,0,1))
        for i in range(3):
            for j in range(3):
                if (Resolution_Degree[i] == 0 && Resolution_Degree[j] == 0):
                    #Do nothing
                    else
                        Neighbour = ((x[0]+Resolution_Degree[i]*self.resolution, x[1]+Resolution_Degree[j]*self.resolution))
                        if (self.is_free(Neighbour_List)):
    ...
    def solve(self):
        while len(self.open_set)>0: #while O.Size > 0 
            # TODO: fill me in!
            # Set current x with lowest f score in O 
            x_current = self.find_best_f_score()
            # if x_current = x_goal then 
            ...
            #C.ADD
            self.closest.append(x_current)

            #for xneigh in neighbours (xcurrent) do
            for x_neighbour in self.get_neighbors(x_current):

            #if xneigh in C then
                if x_neighbour in self.closest:
                    continue
                else:
                    Tantative_G_SCORE = self.g_score[x_current] + self.distance(x_curent,x_neighbour)
            #continue
            #end if
            #tentative g score = GET G SCORE(xcurrent) + DISTANCE(xcurrent, xneigh)
                    if x_neighbour not in self.open_set:
                        self.open_set.append(x_neighbour)
                    elif Tantative_G_SCORE > self.g_score[x_current]:
                        continue
                    #if xneigh not in O then
                    #O.ADD(xneigh)
                    #else if tentative g score > GET G SCORE(xneigh) then
                    #continue
                    #end if
                    #SET CAME FROM(xneigh, xcurrent)
                    self.came_from[x_neighbour] = x_current
                    #SET G SCORE(xneigh, tentative g score)
                    self.g_score[x_neighbour] = Tantative_G_SCORE 
                    #SET F SCORE(xneigh, tentative g score + HEURISTIC(xneigh, xgoal))
                    self.f_score[x_neighbour] = Tantative_G_SCORE + self.distance(x_neighbour,self.x_goal)
            #end for
        #end while
        return False
